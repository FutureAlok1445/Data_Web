...
export function decideChartType(
    ...
    if (suggestedChart) {
        // Map common names to ChartType
        const s = suggestedChart.toLowerCase();
        if (s.includes('bar')) return 'BarChart';
        if (s.includes('line')) return 'LineChart';
        if (s.includes('pie') || s.includes('donut')) return 'PieChart';
        if (s.includes('histogram') || s.includes('distrib')) return 'HistogramChart';
        if (s.includes('scatter')) return 'ScatterChart';
        if (s.includes('heatmap')) return 'HeatmapChart';
    }
