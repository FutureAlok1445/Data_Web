...
export const UploadPanel: React.FC = () => {
    ...
    const handleFile = async (file: File) => {
        ...
        try {
            ...
            // Step 2: Auto-run initial analysis after upload
            setIsUploading(false);
            setIsAnalyzing(true);

            // Compose a smart default question based on the uploaded data
            const targetCol = response.target_column;
            const cols = Object.keys(response.schema || {});
            const autoQuestion = targetCol
                ? `Give me a complete overview: What is the overall ${targetCol.replace(/_/g, ' ')} rate, and how does it vary by the top categorical features?`
                : `Give me an overview of this dataset: show the distribution of the most important columns and key summary statistics.`;

            const userMsg = {
                id: Date.now().toString(),
                role: 'user' as const,
                content: `📊 Auto-analysis: ${autoQuestion}`
            };
            addMessage(userMsg);
            setLoading(true);
            ...
            try {
                ...
                addMessage({
                    id: (Date.now() + 1).toString(),
                    role: 'assistant',
                    content: 'Initial analysis complete. Ask me anything about your data!',
                    isError: false
                });
            } finally {
                setLoading(false);
            ...
            >
                ...
                >
                    ...
                    <h2 className="text-3xl font-black text-slate-800 mb-4">
                        {isAnalyzing ? 'Analyzing your data...' : 'Start your Analysis'}
                    </h2>
                    <p className="text-slate-500 mb-10 max-w-sm mx-auto font-medium leading-relaxed">
                        {isAnalyzing
                            ? 'Running AI-powered analysis and generating visualizations. This will just take a moment.'
                    ...
                    {!isProcessing && (
                        <>
                            ...
                            <label
                                htmlFor="file-upload"
                                className={`inline-flex items-center gap-3 px-10 py-5 bg-slate-900 text-white font-black rounded-2xl shadow-xl hover:bg-blue-600 hover:shadow-blue-200 active:scale-95 cursor-pointer transition-all duration-300`}
                            >
                                BROWSE FILES
                    ...
                    {isProcessing && (
                        <div className="inline-flex items-center gap-3 px-10 py-5 bg-blue-600 text-white font-black rounded-2xl shadow-xl opacity-80 cursor-not-allowed">
